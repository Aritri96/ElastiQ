...
# Calculate C vector
C = np.zeros(6)
C[0] = np.dot(sobs[:, 0], eapp[:, 0]) + np.dot(sobs[:, 1], eapp[:, 1]) + 0.5 * np.dot(sobs[:, 5], eapp[:, 5])
C[1] = np.dot(sobs[:, 0], eapp[:, 1]) + np.dot(sobs[:, 1], eapp[:, 0]) - 0.5 * np.dot(sobs[:, 5], eapp[:, 5])
C[2] = np.dot(sobs[:, 0], eapp[:, 2]) + np.dot(sobs[:, 1], eapp[:, 2]) + np.dot(sobs[:, 2], eapp[:, 0] + eapp[:, 1])
C[3] = (
    np.dot(sobs[:, 0], eapp[:, 3]) -
    np.dot(sobs[:, 1], eapp[:, 3]) +
    np.dot(sobs[:, 3], eapp[:, 0] - eapp[:, 1]) +
    np.dot(sobs[:, 5], eapp[:, 4]) +
    np.dot(sobs[:, 4], eapp[:, 5])
)
C[4] = np.dot(sobs[:, 2], eapp[:, 2])
C[5] = np.dot(sobs[:, 3], eapp[:, 3]) + np.dot(sobs[:, 4], eapp[:, 4])
...
A[1, 1] = np.dot(eapp[:, 0], eapp[:, 0]) + np.dot(eapp[:, 1], eapp[:, 1]) + 0.25 * np.dot(eapp[:, 5], eapp[:, 5])
A[1, 2] = np.dot(eapp[:, 2], eapp[:, 1]) + np.dot(eapp[:, 2], eapp[:, 0])
A[1, 3] = np.dot(eapp[:, 3], eapp[:, 1]) - np.dot(eapp[:, 3], eapp[:, 0]) - 0.5 * np.dot(eapp[:, 4], eapp[:, 5])

A[2, 0] = np.dot(eapp[:, 2], eapp[:, 0]) + np.dot(eapp[:, 2], eapp[:, 1])
A[2, 1] = np.dot(eapp[:, 2], eapp[:, 1]) + np.dot(eapp[:, 2], eapp[:, 0])
A[2, 2] = np.dot(eapp[:, 2], eapp[:, 2]) + np.dot(eapp[:, 0] + eapp[:, 1], eapp[:, 0] + eapp[:, 1])
A[2, 4] = np.dot(eapp[:, 2], eapp[:, 0] + eapp[:, 1])

A[3, 0] = np.dot(eapp[:, 3], eapp[:, 0]) - np.dot(eapp[:, 3], eapp[:, 1]) + 0.5 * np.dot(eapp[:, 4], eapp[:, 5])
A[3, 1] = np.dot(eapp[:, 3], eapp[:, 1]) - np.dot(eapp[:, 3], eapp[:, 0]) - 0.5 * np.dot(eapp[:, 4], eapp[:, 5])
A[3, 3] = (
    np.dot(eapp[:, 3], eapp[:, 3]) +
    np.dot(eapp[:, 0] - eapp[:, 1], eapp[:, 0] - eapp[:, 1]) +
    np.dot(eapp[:, 4], eapp[:, 4]) +
    np.dot(eapp[:, 5], eapp[:, 5])
)

A[4, 2] = np.dot(eapp[:, 2], eapp[:, 0] + eapp[:, 1])
A[4, 4] = np.dot(eapp[:, 2], eapp[:, 2])

A[5, 5] = np.dot(eapp[:, 3], eapp[:, 3]) + np.dot(eapp[:, 4], eapp[:, 4])
