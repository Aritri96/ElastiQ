...
C[6] = (
    np.dot(sobs[:, 0], eapp[:, 5]) -
    np.dot(sobs[:, 1], eapp[:, 5]) +
    np.dot(sobs[:, 5], eapp[:, 0])
)
...
A[6, 0] = np.dot(eapp[:, 0], eapp[:, 5]) + np.dot(eapp[:, 1], eapp[:, 5])
A[6, 1] = np.dot(eapp[:, 0], eapp[:, 5]) + np.dot(eapp[:, 1], eapp[:, 5])
A[6, 2] = 2 * np.dot(eapp[:, 2], eapp[:, 5])
A[6, 6] = np.dot(eapp[:, 0], eapp[:, 0])
