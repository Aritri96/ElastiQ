...
C[3] = np.dot(sobs[:, 1], eapp[:, 1])
C[4] = np.dot(sobs[:, 1], eapp[:, 2]) + np.dot(sobs[:, 2], eapp[:, 1])
C[5] = np.dot(sobs[:, 2], eapp[:, 2])
C[6] = np.dot(sobs[:, 3], eapp[:, 3])
C[7] = np.dot(sobs[:, 4], eapp[:, 4])
C[8] = np.dot(sobs[:, 5], eapp[:, 5])
...
A[1, 3] = np.dot(eapp[:, 1], eapp[:, 0])
A[1, 4] = np.dot(eapp[:, 1], eapp[:, 2])

A[2, 0] = np.dot(eapp[:, 0], eapp[:, 2])
A[2, 1] = np.dot(eapp[:, 1], eapp[:, 2])
A[2, 2] = np.dot(eapp[:, 2], eapp[:, 2]) + np.dot(eapp[:, 0], eapp[:, 0])
A[2, 5] = np.dot(eapp[:, 1], eapp[:, 2])
A[2, 6] = np.dot(eapp[:, 0], eapp[:, 2])

A[3, 1] = np.dot(eapp[:, 1], eapp[:, 0])
A[3, 3] = np.dot(eapp[:, 1], eapp[:, 1])
A[3, 4] = np.dot(eapp[:, 2], eapp[:, 1])

A[4, 1] = np.dot(eapp[:, 1], eapp[:, 2])
A[4, 2] = np.dot(eapp[:, 2], eapp[:, 1])
A[4, 4] = np.dot(eapp[:, 1], eapp[:, 1]) + np.dot(eapp[:, 2], eapp[:, 2])
A[4, 5] = np.dot(eapp[:, 1], eapp[:, 2])

A[5, 2] = np.dot(eapp[:, 2], eapp[:, 0])
A[5, 5] = np.dot(eapp[:, 2], eapp[:, 2])

A[6, 6] = np.dot(eapp[:, 3], eapp[:, 3])

A[7, 7] = np.dot(eapp[:, 4], eapp[:, 4])

A[8, 8] = np.dot(eapp[:, 5], eapp[:, 5])
